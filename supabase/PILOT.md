...
3. Trong Supabase Authentication → URL Configuration, đặt Site URL là `https://phucphamdinh18.github.io/firegroup-mini-ats-demo/` và thêm Redirect URL `https://phucphamdinh18.github.io/firegroup-mini-ats-demo/pilot.html`.
4. Sao chép `pilot-config.example.js` thành `pilot-config.js`. Điền **Project URL** và **publishable key** (hoặc legacy anon key nếu project chưa có publishable key). Đây là hai giá trị dành cho trình duyệt; tuyệt đối không dùng `service_role`, secret API key, database password hay OAuth client secret. Tải `pilot.html`, `supabase/pilot-schema.sql`, `supabase/pilot-config.js` lên GitHub đúng đường dẫn.
